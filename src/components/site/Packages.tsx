...
import day2 from "@/assets/mathura-temple.jpg";
import day3 from "@/assets/gokul-ghat.jpg";
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-3">
          ...
          <Reveal className="lg:row-span-2">
            <article className="relative h-full min-h-[400px] overflow-hidden rounded-2xl shadow-soft">
              <img
                src={brajCircle}
                alt="Braj Mandal map — Vrindavan, Mathura, Govardhan, Gokul, Barsana, Nandgaon"
              ...
              <div className="relative flex h-full flex-col justify-center p-8 text-white">
                ...
                <p className="mt-3 text-sm text-white/80 leading-relaxed">
                  ...
                  Vrindavan, Govardhan &amp; all sacred Braj destinations.
                </p>
                <a
                  href="https://wa.me/919027674560"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-6 w-fit rounded-full bg-white px-6 py-2.5 text-xs font-bold tracking-widest text-foreground hover:bg-brand hover:text-white transition-colors"
                >
                  BOOK NOW
                </a>
              </div>
          ...
          {TOURS.map((t, i) => (
            <Reveal key={t.title} delay={i * 60}>
              <article className="hover-lift group relative aspect-[4/3] overflow-hidden rounded-2xl shadow-soft">
                <img
                  src={t.img}
                  alt={`${t.title} — Braj Miles | ${t.desc}`}
                  loading="lazy"
                  className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
                />
                <div className="absolute inset-0 bg-gradient-to-b from-black/60 via-transparent to-black/50" />
                <h3 className="absolute left-0 right-0 top-4 text-center text-lg font-bold text-white drop-shadow">
                  {t.title}
                </h3>
                <div className="absolute inset-x-4 bottom-4 rounded-md bg-white/85 px-4 py-2 text-center backdrop-blur-sm">
                  <div className="text-xs font-extrabold text-foreground">{t.count}</div>
